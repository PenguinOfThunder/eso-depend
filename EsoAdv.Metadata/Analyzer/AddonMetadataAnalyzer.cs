...
public class AddonMetadataAnalyzer
{
    ...
    public List<Issue> Analyze(AddonMetadataCollection addonCollection)
    {
        ...
        foreach (var addon in addons.OrderBy(a => a.Path))
        {
    ...
    private IEnumerable<Issue> ValidateAddOnFiles(AddonMetadataCollection addonCollection, AddonMetadata addon)
    {
        ...
        if (_settings.CheckProvidedFiles)
        {
            ...
            {
                var filePath = Path.Combine(addonCollection.BasePath, addon.Directory,
                    AddonMetadata.ExpandFileName(file, addonCollection.UserSettings.LastValidLanguage,
                        addOnSettings.Version));
    ...
    private IEnumerable<Issue> ValidateAddOnOptionalDependencies(AddonMetadataCollection addonCollection,
        AddonMetadata addon)
    {
        if (_settings.CheckOptionalDependsOn)
            ...
            {
                var matching = addonCollection.FindMatchingAddons(optDep);
                if (!matching.Any())
                    yield return new Issue
                    {
                        AddOnRef = addon.Name,
    ...
    private IEnumerable<Issue> ValidateAddOnDependencies(AddonMetadataCollection addonCollection, AddonMetadata addon)
    {
        if (_settings.CheckDependsOn)
            ...
            {
                var matching = addonCollection.FindMatchingAddons(reqDep);
                if (!matching.Any())
                {
                    yield return new Issue
                    {
                ...
                else
                {
                    ...
                    var nonBundled = nonTopLevel.Where(ao => addonCollection.GetParentAddon(ao) != addon);
                    if (nonBundled.Any())
                        yield return new Issue
                        {
                            AddOnRef = addon.Name,
                            Severity = IssueSeverity.Warning,
                            Message =
                                $"{addon.Name} depends on modules that are not bundled with it, and not installed as separate AddOns: " +
                                string.Join(", ", nonBundled)
                        };
                }
            }
    }
        ...
        if (_settings.CheckMultipleInstances)
        {
            ...
            var duplicates = addonsByName
                ...
                .Select(g => new Issue
                {
                    AddOnRef = g.Key,
                    Severity = IssueSeverity.Warning,
                    Message = $"Multiple instances of {g.Key} was found in: " +
                              string.Join(", ", g.Value.Select(a => a.Path))
                });
            // issues.AddRange(duplicates);
            foreach (var duplicate in duplicates) yield return duplicate;
        }
    }
            ...
            if (addon.APIVersion != null && addOnSettings.Version != null)
                if (!addon.APIVersion.Any(v => int.Parse(v) >= addOnSettings.Version))
                    yield return new Issue
                    {
                        ...
                        Severity = IssueSeverity.Info,
                        Message =
                            $"{addon.Name} is outdated - supported version(s): {string.Join('-', addon.APIVersion)}"
                    };
        }
    }
