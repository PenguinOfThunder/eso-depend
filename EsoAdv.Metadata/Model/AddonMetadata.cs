...
namespace EsoAdv.Metadata.Model
{
    public class AddonMetadata
    {
        ...
        public List<string> ProvidedFiles { get; } = new List<string>();
        ...
        public static string ExpandendFileName(string filename, string language, string apiVersion)
        {
            filename = filename?.Replace("$(APIVersion)", apiVersion);
        ...
        */
        public bool SatisfiesVersion(string versionSpec)
        {
            ...
            string wantedName = m.Groups["name"]?.Value?.Trim();
            int? wantedVersionNo = null;
            if (int.TryParse(m.Groups["version"]?.Value?.Trim(), out int _version))
            {
                wantedVersionNo = _version;
            }
            // This should be an integer             
            return wantedName == this.Name && (wantedVersionNo == null || this.AddOnVersion >= wantedVersionNo);
        }
        ...
        /**
        <summary>ESO added the AddOnVersion: directive to support its own internal addon versioning by using the value in this directive
        to determine which of two identical addon, library, or dependency folders was newer; your addon folder (or a folder
        that is nested within your addon folder) or the folder that ESO keeps within its internal catalog.</summary>
        ...
        version of the library is preferred. Its value should be a positive integer because ESO uses the C language (<c>atoi</c>)
        function to perform the numeric conversion.

        Note: The text to numeric value conversion (C-language <c>atoi</c>) actually only reads characters up to the first non digit
        ...
        */
        public int? AddOnVersion
        {
            get
            {
                if (int.TryParse(GetMetadataField("AddOnVersion"), out int addOnVersion))
                {
                    return addOnVersion;
                }
                return null;
        ...
        */
        ...
        /// <summary>An author or list of authors of this addon, to be displayed in the addon manager. Can contain spaces and other special characters.</summary>
        ...
        /// After each library/AddOn name you can "optionally" add the >=<$integer> value to do a version check against 
        /// the addon#s txt file tag ## AddOnVersion: <$integer> (e.g. LibAddonMenu-2.0>=28 -> Will check for LibAddonMenu-2.0txt, 
        ...
        /// <summary>A description of the addon that will be displayed in the tooltip for the addon in the addons window list. Can contain spaces and other special characters. </summary>
        public string Description => GetMetadataField("Description");

        /**
        <summary>Starting with the Murkmire Update (100025) the game periodically writes the saved variables for each loaded add-on to disk.
        ...
        It will always wait at least two seconds between writing data to disk and 15 minutes before saving the same file again.
        When the DisableSavedVariablesAutoSaving is set to "1", it won't consider the saved variables of this add-on for autosaving. </summary>
        ...
        */
        ...
        /**
        <summary>Starting with the Elsweyr Update (100027) the game will use this directive to identify library
        and support add-ons and to determine which of the two sections, Add-On or Library, to list these add-ons.</summary>
        ...
        */
        ...
        /**
        <summary>Space-separated list of names for data objects that will be persisted to disk in between UI loads, allowing you to save data that 
        isn't reset when the user logs out or reloads the UI. Usually accessed via ZO_SavedVars, but in essence it's just a series of global tables. 
        However, because they are global tables, their data object names are not specific to, nor are they associated with, any one particular add-on.
        </summary>
        ...
        */
        ...
        /**
        <summary>Documents this addon's version identification. The content can be numbers, characters, spaces and other special characters.</summary>
